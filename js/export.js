...
return{id:c.id,name:c.name,cost:carCost(c),price:ap>0?String(ap):"",onRequest:!(ap>0)}})};
render();
...
lb.items.forEach(it=>{const car=S.warehouse.find(c=>c.id===it.id);if(!car)return;
...
<div class="coll-body">
<div style="color:#778;font-size:10px;line-height:1.5;margin-bottom:8px">Накинь наценку на себестоимость сразу всем — или поправь цену каждой машине. «ПО ЗАПРОСУ» — цена в списке не показывается. Округление до 10 тыс ₽.</div>
...
lb.items.forEach((it,i)=>{h+=`<div class="edit-entry-card">
<div class="edit-entry-title" style="display:flex;justify-content:space-between;gap:8px"><span>🚗 ${esc(it.name)}</span>
...
<div class="btn-action btn-green" style="flex:1;margin:0" onclick="lbSend()">📤 ОТПРАВИТЬ СПИСОК</div>
